...
const telegram = vi.fn<(url: string, init?: RequestInit) => Promise<Response>>(async () => new Response("{}"));
const sentTexts = () => telegram.mock.calls.map(([, init]) => JSON.parse(String(init?.body)) as { chat_id: string; text: string });
